...
function VocabularyLesson({ card, onPrev, onNext, isFirst, isLast, onComplete }) {
  ...
  return (
    <div className="flex flex-col items-center gap-8 animate-fade-in-up">
      ...
      >
        <div
          className="relative w-full h-72 transition-transform duration-700 ease-in-out"
          style={{ transformStyle: 'preserve-3d', transform: flipped ? 'rotateY(180deg)' : 'rotateY(0deg)' }}
        >
          {/* Front */}
          <div
            className="absolute inset-0 bg-gradient-to-br from-blue-500 via-blue-600 to-purple-600 rounded-3xl shadow-2xl flex flex-col items-center justify-center gap-3 px-6 overflow-hidden"
          ...
          >
            ...
            <div className="text-8xl drop-shadow-lg relative z-10 hover:scale-110 transition-transform">{card.emoji}</div>
          ...
          {/* Back */}
          <div
            className="absolute inset-0 bg-gradient-to-br from-orange-400 via-orange-500 to-pink-500 rounded-3xl shadow-2xl flex flex-col items-center justify-center gap-4 px-6 overflow-hidden"
          ...
          >
            ...
            <div className="text-white/80 text-base italic text-center px-4">"{card.sentence}"</div>
          </div>
        </div>
      </div>

      {/* Listen button — pulses before pressed */}
      <button
        onClick={handleSpeak}
...
function ListeningLesson({ card, onPrev, onNext, isFirst, isLast, onComplete }) {
  ...
  return (
    <div className="flex flex-col items-center gap-8 animate-fade-in-up">
      <div className="text-center">
        <span className="inline-block bg-sky-100 text-sky-600 text-xs font-black px-4 py-1.5 rounded-full mb-3 border border-sky-200 uppercase tracking-wide">🎧 Listening</span>
      ...
      >
        <Volume2 size={18} className={playing ? 'animate-pulse' : 'animate-pulse'} />
...
function QuizLesson({ question, onPrev, onNext, isFirst, isLast, onComplete }) {
  ...
  return (
    <div className="flex flex-col items-center gap-8 animate-fade-in-up">
      <div className="text-center">
        <span className="inline-block bg-green-100 text-green-600 text-xs font-black px-4 py-1.5 rounded-full mb-3 border border-green-200 uppercase tracking-wide">🎮 Quiz Ойыны</span>
      ...
      {answered && (
        <div className={`w-full max-w-md px-6 py-4 rounded-2xl text-center font-bold text-sm animate-scale-in ${
          correct ? 'bg-gradient-to-r from-green-100 to-emerald-100 text-green-700 border border-green-200' : 'bg-red-50 text-red-600 border border-red-200'
        }`}>
          {correct
            ? '🎉 Керемет! Дұрыс жауап! +10 ұпай!'
            : '😅 Қате болды. Жасыл — дұрыс жауап.'}
        </div>
      )}
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-blue-50/30 flex flex-col">
      ...
      <div className="bg-white/90 backdrop-blur-md border-b border-gray-100 px-4 sm:px-6 py-3 flex items-center gap-4 shadow-sm">
        ...
        <div className="flex-1 hidden sm:flex items-center gap-3 min-w-0">
          <span className="text-2xl shrink-0 hover:scale-110 transition-transform cursor-default">{course.emoji}</span>
                ...
                {mod.lessons.map(lesson => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`text-base shrink-0 ${isActive ? 'animate-wiggle inline-block' : ''}`}>
        ...
        <main className="flex-1 overflow-y-auto p-6 md:p-12 flex items-start justify-center">
          <div className="w-full max-w-lg">
            {currentLesson?.type === 'vocabulary' && (
              <VocabularyLesson key={activeLesson} card={vocabCard} onPrev={goPrev} onNext={goNext} isFirst={activeLesson === 0} isLast={activeLesson === totalLessons - 1} onComplete={handleComplete} />
            )}
            {currentLesson?.type === 'listening' && (
              <ListeningLesson key={activeLesson} card={vocabCard} onPrev={goPrev} onNext={goNext} isFirst={activeLesson === 0} isLast={activeLesson === totalLessons - 1} onComplete={handleComplete} />
            )}
            {currentLesson?.type === 'game' && (
              <QuizLesson key={activeLesson} question={quizQ} onPrev={goPrev} onNext={goNext} isFirst={activeLesson === 0} isLast={activeLesson === totalLessons - 1} onComplete={handleComplete} />
            )}
          </div>
        </main>
      </div>
